...
## Purpose
Content-ranked recall over a caller's visible memory notes, backed by per-scope indexes — in memory by default, optionally disk-backed — with structural scope isolation.
...
Recall SHALL be backed by per-scope indexes plus a single shared-region index. A
recall query SHALL open only the caller's own-scope index and,
